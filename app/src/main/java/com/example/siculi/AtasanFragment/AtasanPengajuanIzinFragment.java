...
public class AtasanPengajuanIzinFragment extends Fragment {
    ...
    AtasanPengajuanIzinKaryawanAdapter atasanPengajuanIzinKaryawanAdapter;
    ...
    private void getDataIzinKaryawan(){
        ...
        atasanInterface.getAllPengajuanIzinKaryawanProses(userId).enqueue(new Callback<List<IzinModel>>() {
            @Override
            public void onResponse(Call<List<IzinModel>> call, Response<List<IzinModel>> response) {
                if (response.isSuccessful() && response.body().size() > 0) {
                    izinModelList = response.body();
    ...
    private void filter(String text) {
        ...
        atasanPengajuanIzinKaryawanAdapter.filter(filteredList);
