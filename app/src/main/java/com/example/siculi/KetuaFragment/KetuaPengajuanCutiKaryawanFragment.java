...
import com.example.siculi.AtasanFragment.AtasanHistoryPengajuanCutiKaryawanFragment;
import com.example.siculi.AtasanFragment.KetuaHistoryPengajuanCutiKaryawanFragment;
...
public class KetuaPengajuanCutiKaryawanFragment extends Fragment {
    ...
    private void getCutiKaryawan(){
        ...
        ketuaInterface.getAllPengajuanCutiKaryawanKetua(userId).enqueue(new Callback<List<CutiModel>>() {
            @Override
            public void onResponse(Call<List<CutiModel>> call, Response<List<CutiModel>> response) {
                if (response.isSuccessful() && response.body().size() > 0) {
                    cutiModelList = response.body();
